...
const pendingCodeAdditions = new Map();
...
const ID = {
  ...
  CODES_MODULE1_BUTTON: "poker_codes_m1_button:",
  CODES_MODULE2_BUTTON: "poker_codes_m2_button:",
  CODES_MODULE1_MODAL: "poker_codes_m1_modal:",
  CODES_MODULE2_MODAL: "poker_codes_m2_modal:",
  ADD_CODES_MODULE1_BUTTON: "poker_add_codes_m1_button:",
  ADD_CODES_MODULE2_BUTTON: "poker_add_codes_m2_button:",
  ADD_CODES_MODULE1_MODAL: "poker_add_codes_m1_modal:",
  ADD_CODES_MODULE2_MODAL: "poker_add_codes_m2_modal:",
...
function buildFinalSetupComponents(pendingId) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`${ID.CODES_MODULE1_BUTTON}${pendingId}`)
        .setLabel("Codes Module 1")
        .setEmoji("🔐")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId(`${ID.CODES_MODULE2_BUTTON}${pendingId}`)
        .setLabel("Codes Module 2")
        .setEmoji("🔐")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
...
async function handleCodesButton(interaction, pendingId, moduleIndex) {
  const modal = new ModalBuilder()
    .setCustomId(
      `${moduleIndex === 0 ? ID.CODES_MODULE1_MODAL : ID.CODES_MODULE2_MODAL}${pendingId}`
    )
    .setTitle(`Add Codes Module ${moduleIndex + 1}`);
  const codesInput = new TextInputBuilder()
    .setCustomId("code_list")
    .setLabel("One code per line (max 4000 characters)")
...
async function handleCodesModal(interaction, pendingId, moduleIndex) {
  await interaction.deferReply({ ephemeral: true });
  ...
  const codes = interaction.fields
    ...
    .map((code) => code.trim())
    .filter(Boolean);
  const otherModuleIndex = moduleIndex === 0 ? 1 : 0;
  const totalCodes = codes.length + pending.codeModules[otherModuleIndex].length;
  if (totalCodes > pending.maxEntries) {
    await interaction.editReply(
      `❌ These modules would contain ${totalCodes} codes, but the tournament limit is ${pending.maxEntries}.`
    );
    return;
  }
  pending.codeModules[moduleIndex] = codes;
  pending.codes = [
    ...pending.codeModules[0],
    ...pending.codeModules[1],
  ];
  await interaction.editReply({
    content:
...
function buildAddCodesComponents(sessionId) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`${ID.ADD_CODES_MODULE1_BUTTON}${sessionId}`)
        .setLabel("Add Codes — Module 1")
        .setEmoji("🔐")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId(`${ID.ADD_CODES_MODULE2_BUTTON}${sessionId}`)
        .setLabel("Add Codes — Module 2")
        .setEmoji("🔐")
        .setStyle(ButtonStyle.Secondary)
    ),
  ];
...
async function handleAddCodesButton(interaction, sessionId, moduleIndex) {
  const session = pendingCodeAdditions.get(sessionId);
  if (
    !session ||
    !interaction.guild ||
    interaction.guild.id !== session.guildId ||
    interaction.user.id !== session.userId
  ) {
    await interaction.reply({
      content: "❌ This code-add session expired or belongs to another user.",
      ephemeral: true,
    });
    return;
  }
  ...
  const modal = new ModalBuilder()
    .setCustomId(
      `${moduleIndex === 0 ? ID.ADD_CODES_MODULE1_MODAL : ID.ADD_CODES_MODULE2_MODAL}${sessionId}`
    )
    .setTitle(`Add More Codes — Module ${moduleIndex + 1}`);
  const codesInput = new TextInputBuilder()
    .setCustomId("code_list")
    .setLabel("One code per line (max 4000 characters)")
    .setStyle(TextInputStyle.Paragraph)
    .setPlaceholder("CODE-401\nCODE-402\nCODE-403")
    .setRequired(true)
    .setMaxLength(4000);
  modal.addComponents(new ActionRowBuilder().addComponents(codesInput));
  await interaction.showModal(modal);
...
async function handleAddCodesModal(interaction, sessionId, moduleIndex) {
  await interaction.deferReply({ ephemeral: true });
  ...
  if (
    ...
    await interaction.editReply("❌ This code-add session expired.");
    return;
  }

  const newCodes = interaction.fields
    .getTextInputValue("code_list")
    .split(/\r?\n/)
    .map((code) => code.trim())
    .filter(Boolean);
  if (!newCodes.length) {
    await interaction.editReply("❌ Please provide at least one code.");
    return;
  }

  const tournament = data.tournaments.find(
    (item) =>
      item.id === session.tournamentId &&
      item.guildId === session.guildId &&
      item.status === "active"
  );
  if (!tournament) {
    await interaction.editReply("❌ Tournament not found or registration is closed.");
    pendingCodeAdditions.delete(sessionId);
    return;
  }

  const existingCodes = Array.isArray(tournament.codes) ? tournament.codes : [];
  const totalCodes = existingCodes.length + newCodes.length;
  if (Number(tournament.maxEntries) > 0 && totalCodes > tournament.maxEntries) {
    await interaction.editReply(
      `❌ This would make **${totalCodes}** codes, but the tournament limit is **${tournament.maxEntries}**.`
    );
    return;
  }

  tournament.codes = [...existingCodes, ...newCodes];
  saveData(data);
  await interaction.editReply({
    content:
...
async function handleRegisterModal(interaction, tournamentId, client) {
  ...
  const assignedCodeCount = data.registrations.filter(
    (item) => item.tournamentId === tournamentId && item.assignedCode
  ).length;
  const registration = {
    id: crypto.randomUUID(),
    tournamentId,
    ...
    assignedCode: Array.isArray(tournament.codes)
      ? tournament.codes[assignedCodeCount] || null
      : null,
...
client.on("messageCreate", async (message) => {
  ...
  return;
    ...
    pendingCodeAdditions.set(sessionId, {
      guildId: message.guild.id,
      channelId: message.channel.id,
      userId: message.author.id,
      tournamentId: tournament.id,
    });
    const currentCodes = Array.isArray(tournament.codes)
      ? tournament.codes.length
      : 0;
    const remaining =
      Number(tournament.maxEntries) > 0
        ? `Remaining capacity: **${Math.max(Number(tournament.maxEntries) - currentCodes, 0)}**`
        : "Remaining capacity: **Unlimited**";
    const embed = new EmbedBuilder()
      .setColor(0x5865f2)
      .setTitle("🔐 Add More Tournament Codes")
      .setDescription(
        `Tournament: **${tournament.pokerName}**\n` +
        `ID: \`${tournament.id}\`\n\n` +
        "Choose a module and paste one code per line. Each submission is appended after the existing codes.\n" +
        "Each module accepts up to **4,000 characters**."
      )
      .addFields(
        { name: "Codes already loaded", value: `**${currentCodes}**`, inline: true },
        { name: "Capacity", value: remaining, inline: true }
      )
...
client.on("interactionCreate", async (interaction) => {
  try {
    if (interaction.isButton()) {
      ...
      } else if (interaction.customId.startsWith(ID.CODES_MODULE1_BUTTON)) {
        await handleCodesButton(
          interaction,
          interaction.customId.slice(ID.CODES_MODULE1_BUTTON.length),
          0
        );
      } else if (interaction.customId.startsWith(ID.CODES_MODULE2_BUTTON)) {
        await handleCodesButton(
          interaction,
          interaction.customId.slice(ID.CODES_MODULE2_BUTTON.length),
          1
        );
      } else if (interaction.customId.startsWith(ID.ADD_CODES_MODULE1_BUTTON)) {
        await handleAddCodesButton(
          interaction,
          interaction.customId.slice(ID.ADD_CODES_MODULE1_BUTTON.length),
          0
        );
      } else if (interaction.customId.startsWith(ID.ADD_CODES_MODULE2_BUTTON)) {
        await handleAddCodesButton(
          interaction,
          interaction.customId.slice(ID.ADD_CODES_MODULE2_BUTTON.length),
          1
        );
      ...
      } else if (interaction.customId.startsWith(ID.CODES_MODULE1_MODAL)) {
        await handleCodesModal(
          interaction,
          interaction.customId.slice(ID.CODES_MODULE1_MODAL.length),
          0
        );
      } else if (interaction.customId.startsWith(ID.CODES_MODULE2_MODAL)) {
        await handleCodesModal(
          interaction,
          interaction.customId.slice(ID.CODES_MODULE2_MODAL.length),
          1
        );
      } else if (interaction.customId.startsWith(ID.ADD_CODES_MODULE1_MODAL)) {
        await handleAddCodesModal(
          interaction,
          interaction.customId.slice(ID.ADD_CODES_MODULE1_MODAL.length),
          0
        );
      } else if (interaction.customId.startsWith(ID.ADD_CODES_MODULE2_MODAL)) {
        await handleAddCodesModal(
          interaction,
          interaction.customId.slice(ID.ADD_CODES_MODULE2_MODAL.length),
          1
        );
